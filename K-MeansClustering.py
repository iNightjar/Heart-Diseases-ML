...
def draw_clusters(table):
    ...
    pca_df = pd.DataFrame(
        new_data, columns=['principal component1', 'principal component2'])

    # drawing
    sns.scatterplot(x="principal component1",
                    y="principal component2", hue=table['cluster'], data=pca_df)
    plt.title('K-means Clustering in 2D')
    # Edit 'path_to_save' as your directory to save the result diagram
    plt.savefig('/ home/inightjar/ConTainer/codehub/Heart-Diseases-ML/kmeans_result.png')
    plt.show()
